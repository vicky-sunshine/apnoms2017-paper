...
\title{A Novel vCPE Framework for Enabling Virtual Network Functions with Multiple Flow Tables Architecture in SDN
switches}
...
Inspired from these two frameworks, we proposed a vCPE framework and six major virtual network functions in home gateway, attempting to replace hardware-based CPE.
Furthermore, when the NFV is performed by SDN switch, there will be restricting on the OpenFlow Table \cite{multiple-flow-table}, thus we also proposed a multiple OpenFlow table mechanism to implement network functions and explain how to use it to resolve the restriction of table.
