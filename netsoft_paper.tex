...
Our network functions (Fig. \ref{fig:desc_nfv_overview}) are designed with SDN-enabled NFV architecture concept \cite{sdn-enabled}, using the synergies between computer infrastructures (NFVs) and network infrastructures (NFVIs) \cite{etsi-nfv-terminology, etsi-nfv-infra}.
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.45\textwidth]{./figures/desc_service_deployment}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.45\textwidth]{./figures/desc_vcpe_framework}
...
% As shown in Table \ref{table:fw}, all the actions in flow entries are dropped. The first rule illustrates that SSH connection with the source IP address 192.168.2.1 is blocked. The second rule indicates that the flow entry blocks the Telnet protocol.
...
% In Fig. \ref{fig:mft_nat}, the public IP address of NAT is 140.114.71.178, and the host private IP address is 192.168.8.254 with the port number 7878.
% The client sent the packet to a server with the IP address 140.114.71.177 and port number 9898.
